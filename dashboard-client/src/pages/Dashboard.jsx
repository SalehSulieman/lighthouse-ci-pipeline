import React, { useState, useEffect } from 'react';import { getMetrics, getTrendAnalysis, getAlerts } from '../api';import PerformanceDashboard from '../components/PerformanceDashboard';import './Dashboard.css';

const Dashboard = () => {const [latestMetrics, setLatestMetrics] = useState(null);const [trends, setTrends] = useState(null);const [activeAlerts, setActiveAlerts] = useState([]);const [selectedEnv, setSelectedEnv] = useState('production');

// ADDED: State to control the artificial delayconst [isHeavyContentLoaded, setIsHeavyContentLoaded] = useState(false);
...
const loadData = async () => {const data = await getMetrics(selectedEnv);if (data && data.length > 0) {setLatestMetrics(data[0]);} else {setLatestMetrics(null);}
...
const formatSeconds = (seconds) => {if (!seconds) return '0.00';return seconds.toFixed(2);};

const getSystemStatus = () => {if (!latestMetrics) return { text: "No Data", icon: "⚪", color: "#64748b", bg: "#f1f5f9" };
...
const renderTrend = (metricKey) => {if (!trends || !trends.deltas || !trends.deltas[metricKey]) return null;const trend = trends.deltas[metricKey];const isGood = trend.isImprovement;
